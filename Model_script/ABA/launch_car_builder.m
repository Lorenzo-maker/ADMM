...
car = car_builder(alpha_vec, pista, colloc_type, d);

function car = car_builder(alfarange, pista, colloc_type, d)
...
%% Evaluate numerical quantities & save in property.data (maybe they can be evaluated in sub_opti_map ?)
    pos_map = pista.fun_pos.map(length(alfarange),'thread',4);
    car.data.track.pos = pos_map(alfarange);
    % Build map version
    g_gs_map = pista.fun_g_gs.map(length(alfarange),'thread',4);
    T_jac_map = car.T_jac.map(length(alfarange),'thread',4);
    T_jac_der_map = car.T_jac_der.map(length(alfarange),'thread',4);
    % Evaluate numerical quantities at grid points
    g_gs_num = full(g_gs_map(alfarange));
    g_gs_num = reshape(g_gs_num, 4, 4, []);
    Jac_num = full(T_jac_map(alfarange));
    Jac_der_num = chop(full(T_jac_der_map(alfarange)),1);
    car.data.track.g_gs_num = g_gs_num;
    car.data.track.Jac_num = Jac_num;
    car.data.track.Jac_der_num = Jac_der_num;
    
    
    
    % Evaluate numerical quantities at collocation
    pre = diff(alfarange);
    index = find(pre<0);
    if ~isempty(index)
    ...
    dalfa_vec = pre; 
    car.data.track.dalfa_vec = dalfa_vec;
    ...
    for j = 1:d
        alfa_colloc = min(alfarange(1:end-1) + dalfa_vec(1:end)*tau_colloc(j),1);
        g_gs_colloc_j = full(g_gs_map([0,alfa_colloc]));        
        g_gs_colloc_j = reshape(g_gs_colloc_j(:,5:end),4,4,[]);
        Jac_colloc_j = full(T_jac_map([0,alfa_colloc]));
        Jac_colloc_j = Jac_colloc_j(:,2:end);
        Jac_der_colloc_j = chop(full(T_jac_der_map([0,alfa_colloc])),1);
        Jac_der_colloc_j = Jac_der_colloc_j(:,2:end);
        for i = 1:length(alfarange)-1
            g_gs_colloc{i,j} = g_gs_colloc_j(:,:,i);
            Jac_colloc{i,j} = Jac_colloc_j(:,i);
            Jac_der_colloc{i,j} = Jac_der_colloc_j(:,i);            
        end            
    end
    car.data.track.g_gs_colloc = g_gs_colloc;
    car.data.track.Jac_colloc = Jac_colloc;
    car.data.track.Jac_der_colloc = Jac_der_colloc;
