...
dist = ceil(min(vecnorm(dist')));
guess.dt = dist/(guess.u);%0.5*model.opt.dt_max;% 1.4*dist/(guess.u);

% Guessing front slip angle
guess.alphayf = 0.05*if_else_smooth(guess.Fyf,0,1,-1,'C',0.0001);%sign(guess.Fyf);
