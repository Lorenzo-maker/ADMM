function [problema, numericalData, scale]  = sub_opti_map(alfarange, pista, o, id, ID, problem_number, d, init_subrange, alpha_vec, opts, car)
...
% tail (no controls, no algebraic variables)
    ...
    if length(ID.H) > 1
        if isempty(ID.H{problem_number}) && isempty(ID.h{problem_number})
            ...
            psi_grid = model.track.psi_grid(1:max(out));
        elseif isempty(ID.T{problem_number}) && isempty(ID.t{problem_number})
            in = [ID.H{problem_number},ID.h{problem_number}];
            psi_grid = model.track.psi_grid(min(in):end);
        else
            in = [ID.H{problem_number},ID.h{problem_number}];
            out = [ID.T{problem_number},ID.t{problem_number}];
            psi_grid = model.track.psi_grid(min(in):max(out));
        end
    else
        psi_grid = model.track.psi_grid;
    end
    ...
    pb.Jj = pb.Jj*activation_opt + activation_start*(pb.z(13)*sys.parameters.ep_scaling)^2;
    ...
    % beta_r
    pb.append_g(pb.z(10) - pb.u(2), -10, 0);
    % gamma_f
    pb.append_g(pb.z(11) - pb.u(1), -10, 0);
    % gamma_r
    pb.append_g(pb.z(12) - pb.u(1), -10, 0);
    %adherence_f
    ...
    %ep
    pb.append_g(pb.z(13)*sys.parameters.ep_scaling, pb.pgb(1), pb.pgb(2));
    %track
    pos = pb.pg(10:11);
    n = pb.pg(12:13);
    pb.append_g(pb.x(4:5) - (pos + n*pb.z(13)*sys.parameters.ep_scaling)./sys.states.X_scale(4:5), zeros(2,1), zeros(2,1))
    %Eq
    ...
    %Cyclic constraint
    % pb.append_g([pb.x0(1:3);pb.z0(13)]-[pb.x_end(1:3);pb.z_end(13)], zeros(4,1), zeros(4,1), 'cyclic')
    % Build map function 
    pb.build_map;
    % Build g 
    pb.build_g;
