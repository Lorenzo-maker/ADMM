function track = track2casadiSpline_split(track, alpha_lim)
%
%
%

alpha_start = alpha_lim(1);
alpha_end = alpha_lim(2);

alpha_vec = linspace(alpha_start, alpha_end, 200);
pts = full(track.fun_pos(alpha_vec));
...
x_interp = casadi.interpolant('interp', 'bspline', {alpha_vec}, x_data);
y_interp = casadi.interpolant('interp', 'bspline', {alpha_vec}, y_data);
z_interp = casadi.interpolant('interp', 'bspline', {alpha_vec}, z_data);
wl_interp = casadi.interpolant('interp', 'bspline', {alpha_vec}, wl_data);
wr_interp = casadi.interpolant('interp', 'bspline', {alpha_vec}, wr_data);
tw_interp = casadi.interpolant('interp', 'bspline', {alpha_vec}, tw_data);
