...
kp_grid_colloc = full(track.fun_kp(alfa_grid_colloc(1:2,1:end-1))); % N dimension
ts_grid = full(track.fun_ts(alfa_grid));
ts_grid(3,:) = 0;
normts = full(track.fun_normts(alfa_grid));
normts_colloc = full(track.fun_normts(alfa_grid_colloc(1:2,1:end-1))); % N dimension
ts_grid = ts_grid./vecnorm(ts_grid);
psi_grid = atan_track(ts_grid, 'clockwise');
RVSG = cell(1,N+1);
RVSG_colloc = cell(d,N+1);
for i = 1:N+1
    RVSG{i} = (full(track.fun_Rgs(alfa_grid(i))))';
    for j = 1:d
        RVSG_colloc{j, i} = (full(track.fun_Rgs(alfa_grid_colloc(j+1,i))))';
    end
end
...
data.ep_scale = 10;
data.h_scale = 1;
data.X_scale = [data.u_scale; data.v_scale; data.r_scale; data.ep_scale; data.ef_scale; repmat(data.w_scale,4,1)];
data.U_scale = [data.Ta_scale; data.Tb_scale; data.delta_scale];
data.Z_scale = repmat(data.Fz_scale,4,1);

% Car Model
car = vehicle_casadi('double-track-spatial', data);
% Intial Condition
car.Xb.x0 = [car.data.Vi;0;0;0;0;repmat(car.data.Vi/car.data.rw,2,1); repmat(car.data.Vi*(1 - 0)/car.data.rw,2,1)]./car.data.X_scale;
car.Xb.x0_lb = [car.data.Vi;0;0;0;0;repmat(car.data.Vi/car.data.rw,2,1); repmat(car.data.Vi*(1 + 0.05)/car.data.rw,2,1)]./car.data.X_scale;
car.Xb.x0_ub = [car.data.Vi;0;0;0;0;repmat(car.data.Vi/car.data.rw,2,1); repmat(car.data.Vi*(1 + 0.05)/car.data.rw,2,1)]./car.data.X_scale;
...
Pdata.Pg = [kp_grid(2:end); kp_grid_colloc; normts(2:end); normts_colloc; Rvsg(:,2:end); Rvsg_colloc];

% NLP
pb = nlp_casadi_2(car.nx, car.nu, car.nz, car.nuc, car.nzc, car.nwg, N, d, car.Xb, car.Ub, car.Zb, [], [], [], 2 + 9 + d + d + 9*d, 2, Pdata, 8, 0, 'SXMX');
pb.w0 = [];
for k = 1:N
    initial_guess_k_spatial;    
    pb.w0 = [pb.w0;w0_k];
end
...
% extract data
p_num = pb.pg;
knKj = reshape(p_num(1:1+d),1, d+1); p_num = p_num(1+d + 1:end);
normtsKj = reshape(p_num(1:1+d),1, d+1); p_num = p_num(1+d + 1:end);
RvsgKj = reshape(p_num, 9, d+1);
% NLP
pb.append_g(pb.x_1(8) - pb.x_1(9), 0, 0, 'start');
for i = 1:d  
   Rvgsj = reshape(RvsgKj(:, i+1), 3,3);
   knj = knKj(i+1);
   normtsj = normtsKj(i+1);
   g3DKj = car.fun_g3D(pb.xc(5,i),Rvgsj);
   qj = car.L(pb.xc(:,i), knj, normtsj);
   pb.append_g(car.F(pb.xc(:,i), pb.u, pb.z, g3DKj, knj, normtsj)*dalfa - pb.xp(:,i).*car.data.X_scale, zeros(car.nx,1), zeros(car.nx,1)) ;
end
RvgsK = reshape(RvsgKj(:, 1), 3,3);
knK = knKj(1);
normtsK = normtsKj(1);
g3DK = car.fun_g3D(pb.x(5), RvgsK); 
xdot = car.F(pb.x, pb.u, pb.z, g3DK, knK, normtsK);
qk = car.L(pb.x, knK, normtsK);
pb.Jj = (qk*dalfa)^2 + 10*((pb.u(3) - pb.u_1(3))*car.data.tau)^2 + 0.0001*(xdot(2)/(qk))^2 + 0.01*(xdot(3)/qk)^2 + 0*0.000001*(xdot(end-4)^2 + xdot(end-3)^2 + xdot(end-2)^2 + xdot(end-1)^2);
% Continuity equation for generic interval
pb.append_g(pb.xc_end - pb.x , zeros(car.nx,1), zeros(car.nx,1));
% Stay on track constraint
pb.append_g(pb.x(4), pb.pgb(1)/car.data.X_scale(4), pb.pgb(2)/car.data.X_scale(4));    
% Algebraic equations constraints
pb.append_g(car.Eq(pb.x, pb.u, pb.z, g3DK, knK, normtsK), zeros(car.neq,1), zeros(car.neq,1));
% Steering constraint
...
% Optimal time
h_opt = car.L(X_opt(:,1:end)./car.data.X_scale, kp_grid(1:end), normts(1:end)).*dalfa;
time_opt = [0, cumsum(full(h_opt))];
...
hold on
grid on
middle = full(track.fun_pos(alfa_grid));
xv = middle(1,:) + X_opt(4,:).*ns_grid(1,:);
yv = middle(2,:) + X_opt(4,:).*ns_grid(2,:);
plot(xv, yv, 'Linewidth', 2, 'color', [0, .8, .1]);
...
% wheel
figure(3)
plot(alfa_grid, X_opt(6,:), alfa_grid, X_opt(7,:), 'o', alfa_grid, X_opt(8,:), alfa_grid, X_opt(9,:),'*')
title('wij')
legend('$w_{11}$','$w_{12}$','$w_{21}$','$w_{22}$','Orientation','horizontal')
...
title('Fxij')
legend('$Fx_{11}$','$Fx_{12}$','$Fx_{21}$','$Fx_{22}$','Orientation','horizontal')
...
plot(xv, yv, 'color', [0 0.5 0.5], 'Linewidth', 2);
