...
pb = nlp_casadi_2(nx, nu, nz, nuc, nzc, 0, N, d, Xb, Ub, Zb, [], [], [], 1, 2, Pdata, 1, 0, 'SX');
...
Z_0 = (1/N)*ones(1,N);
pb.w0 = [U_0; Z_0; repmat(X_0, d, 1); repmat(U_0, d, nuc); repmat(U_0, d, nzc); X_0];
pb.w0 = [X_0(:,1); pb.w0(:)];
...
for i = 1:d
   pb.append_g(F(pb.xc(:,i), pb.u )*pb.z - pb.xp(:,i), zeros(nx,1), zeros(nx,1)) ;
   pb.Jj = [pb.Jj; L(pb.z)];
end
...
sol.x_opt     = full(sol_sym.x(1:(nx+nz+nu+(nx+nuc+nzc)*d):siz));
sol.y_opt     = full(sol_sym.x(2:(nx+nz+nu+(nx+nuc+nzc)*d):siz));
sol.v_opt     = full(sol_sym.x(3:(nx+nz+nu+(nx+nuc+nzc)*d):siz));
sol.teta_opt  = full(sol_sym.x(4:(nx+nz+nu+(nx+nuc+nzc)*d):siz));
sol.h_opt     = full(sol_sym.x(5:(nx+nz+nu+(nx+nuc+nzc)*d):siz));
sol.teta1_opt = full(sol_sym.x(12:(nx+nz+nu+(nx+nuc+nzc)*d):siz));
sol.teta2_opt = full(sol_sym.x(13:(nx+nz+nu+(nx+nuc+nzc)*d):siz));
sol.time_opt  = [0; cumsum(sol.h_opt)];
close all
