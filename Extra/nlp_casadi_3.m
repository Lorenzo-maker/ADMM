classdef nlp_casadi_3 < handle
    ...
    methods
        function obj = nlp_casadi_3(nx, nu, nz, nuc, nzc, nwg, N, d, Xb, Ub, Zb, Ucb, Zcb, Wgb, Npg, Npgb, Pdata, Nthread, np, sym_type, corrected, nq)
            arguments
                nx = 1; nu = 1; nz = 1; nuc = 0; nzc = 0; nwg = 0; N = 1; d = 2; Xb = []; Ub = []; Zb = []; ...
                Ucb = []; Zcb = []; Wgb = []; Npg = 0; Npgb = 0; Pdata = []; Nthread = 1; np = 0; sym_type = 'SX'; corrected = 0; nq = 0;
            end
            import casadi.*
        ...
        function run_colloc(obj)
            if obj.corrected
                obj.col_points_corrected()
            else
                obj.col_points()
            end
        end
        ...
        function col_points(obj)
            ...
            if obj.d == 0
                ...
                tau_root = [0 collocation_points(obj.d, 'radau')];
        ...
        function col_points_corrected(obj)
            % Collocation points definition
            import casadi.*
            if obj.d == 0
                obj.B = [1,1];
                ...
                % Get collocation points
                tau_root = [0 collocation_points(obj.d, 'radau')];
                pxc = SX.sym('pxc', obj.d);
                px0 = SX.sym('px0', 1);
                px00 = SX.sym('px00', 1);
                Coeff = zeros(obj.d+1, obj.d+1);% row = number of points (weight of lagrange), column = number of coefficients for a d-degree poly
                ICoeff = zeros(obj.d+1, obj.d+2);% row = number of points (weight of lagrange), column = number of coefficients for a d-degree poly
                DCoeff = zeros(obj.d+1, obj.d);% row = number of points (weight of lagrange), column = number of coefficients for a d-degree poly
                obj.B = zeros(obj.d+1, 1);
                % Construct polynomial basis
                for j=1:obj.d+1
                    % Construct Lagrange polynomials to get the polynomial basis at the
                ...
                Coeff(j,:) = coeff;
                ICoeff(j,:) = polyint(coeff);
                DCoeff(j,:) = polyder(coeff);
                obj.B(j) = polyval(ICoeff(j,:), 1);                 
                end
                pc = sum(Coeff.*[px0; pxc]);
                pic = sum(ICoeff.*[px0; pxc]);
                %pic(end) = px00;
                pdc = sum(DCoeff.*[px0; pxc]);
                tau = SX.sym('tau');
                pol = polyval(pc, tau);
                poli = polyval(pic, tau);
                pold = polyval(pdc, tau);
                fp = Function('fp', {tau, px0, pxc},{pol});
                fpi = Function('fpi', {tau, px00, px0, pxc},{poli});
                fpd = Function('fpd', {tau, px0, pxc},{pold});
                % Build vector of collocation
                i = 1;
                while i + obj.nq <= obj.nx
                    xccj{i} = fpi(tau_root(2:end), 0, obj.x_1(obj.nq + i), obj.xc(obj.nq + i,:));
                    i = i+1;
%                     for j = 1:obj.d
%                         obj.xcc(i,j) = fpi(tau_root(j+1), obj.x_1(i), obj.x_1(obj.nq + i), obj.xc(obj.nq + i,:));
%                     end
                end
                clear i
                obj.xcc = obj.Jac*vertcat(xccj{:});
                %obj.xcc(obj.nq+1:end, :) = obj.xc(obj.nq + 1:end,:); % corrected collocation points (q, qdot, x)
                
                % Build vector of derivative qdotdot and xdot
                for i = 1:obj.nx - obj.nq
                    xpj{i} = fpd(tau_root(2:end), obj.x_1(obj.nq + i), obj.xc(obj.nq + i,:));
%                     for j = 1:obj.d
%                         obj.xp(i,j) = fpd(tau_root(j+1), obj.x_1(obj.nq + i), obj.xc(obj.nq + i,:));
%                     end
                end
                obj.xp = vertcat(xpj{:});
                % Build vector of poly end values
                i = 1;
                while i + obj.nq <= obj.nx
                    xc_endi{i} = fpi(1, 0, obj.x_1(obj.nq + i), obj.xc(obj.nq + i,:));
                    i = i+1;
                    %obj.xc_end(i) = fpi(1, obj.x_1(i), obj.x_1(obj.nq + i), obj.xc(obj.nq + i,:));
                end
                xx = obj.Jac*vertcat(xc_endi{:});
                clear xc_endi
                clear i
                for i = 1:obj.nq
                    xc_endi{i} = xx(i);
                end
                for i = obj.nq + 1:obj.nx
                    xc_endi{i} = fp(1, obj.x_1(i), obj.xc(i,:));
                    %obj.xc_end(i) = fp(1, obj.x_1(i), obj.xc(i,:));
                end                   
                obj.xc_end = vertcat(xc_endi{:});
            end
        end
        ...
        function casadi_sym(obj)
            ...
            obj.xcc = casadi.(sym_type_local).sym('xc', obj.nq, obj.d);   
            obj.xc_end = casadi.(sym_type_local).sym('xc', obj.nx);               
            obj.xp = casadi.(sym_type_local).sym('xc', obj.nx - obj.nq, obj.d);   
            ...
            %obj.Jac = casadi.(sym_type_local).sym('Jac', obj.nq, obj.nx - obj.nq); 
        end
